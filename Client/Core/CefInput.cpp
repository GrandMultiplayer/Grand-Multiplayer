...
{
	Hook = (WNDPROC)SetWindowLongPtr(FindWindowA(NULL, "Grand Theft Auto V"), GWLP_WNDPROC, (LONG_PTR)HookedWndProc);
	LOG_DEBUG("[CefInput] Hook Installed.");
}
...
{
	if (CefInput::IsVisible() && CCore::Initialized && CGui::IsCursorVisible())
	{
		...
		case WM_MBUTTONDBLCLK: {
			...
			auto browser = CefMainContext::getBrowser();
		...
		else if (uMsg == WM_INPUT && CGui::IsCursorVisible())
		{
			return CallWindowProc(CefInput::GetHook(), hWnd, uMsg, wParam, lParam);
		}
