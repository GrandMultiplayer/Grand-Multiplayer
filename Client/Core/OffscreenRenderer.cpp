...
OffscreenRenderer::OffscreenRenderer(const Settings& settings)
	: settings_(settings),
	initialized_(false),
	texture_id_(0),
	view_width_(0),
...
void OffscreenRenderer::OnPopupShow(CefRefPtr<CefBrowser> browser, bool show)
{
	if (!show)
	{
...
void OffscreenRenderer::OnPopupSize(CefRefPtr<CefBrowser> browser,
	const CefRect& rect)
{
...
CefRect OffscreenRenderer::GetPopupRectInWebView(const CefRect& original_rect)
{
...
void OffscreenRenderer::ClearPopupRects()
{
	popup_rect_.Set(0, 0, 0, 0);
	original_popup_rect_.Set(0, 0, 0, 0);
}

void OffscreenRenderer::OnPaint(CefRefPtr<CefBrowser> browser,
...
void OffscreenRenderer::SetSpin(float spinX, float spinY)
{
	spin_x_ = spinX;
	spin_y_ = spinY;
}

void OffscreenRenderer::IncrementSpin(float spinDX, float spinDY)
{
	spin_x_ -= spinDX;
	spin_y_ -= spinDY;
}