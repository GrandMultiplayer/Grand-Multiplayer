...
Scripts::ScriptInfo scriptnames[] = {
	...
	{"paradise"},
	{"paradise2"},
	//{"pausemenu"}, // Disabling this = crash with pausemenu
	//{"pausemenu_example"},
	//{"pausemenu_map"},
...
void Scripts::StopAll()
{
	if (!CConfig::GetEditor())
	{
		...
		SCRIPT::SET_SCRIPT_AS_NO_LONGER_NEEDED("rerecord_recording");
		PLAYER::FORCE_CLEANUP_FOR_ALL_THREADS_WITH_THIS_NAME("rerecord_recording", 8);
		GAMEPLAY::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME("rerecord_recording");
	}

	for (unsigned int i = 0; i < SizeOfArray(scriptnames); i++)
	{
		if (!SCRIPT::DOES_SCRIPT_EXIST(scriptnames[i].name))
		{
			LOG_ERROR("Script %s doe not exist", scriptnames[i].name);
		}
...
void Scripts::Pulse()
{
	for (unsigned int i = 0; i < SizeOfArray(scriptnames); i++)
	{
		if (SCRIPT::HAS_SCRIPT_LOADED(scriptnames[i].name))
		{
			//LOG_DEBUG("Script %s has loaded, shutting it down....", scriptnames[i].name);

			PLAYER::FORCE_CLEANUP_FOR_ALL_THREADS_WITH_THIS_NAME(scriptnames[i].name, 1); //Cleanup first
			PLAYER::FORCE_CLEANUP_FOR_ALL_THREADS_WITH_THIS_NAME(scriptnames[i].name, 8); //Cleanup first [OUR DEFAULT]
			GAMEPLAY::TERMINATE_ALL_SCRIPTS_WITH_THIS_NAME(scriptnames[i].name); //Terminate
			SCRIPT::SET_SCRIPT_AS_NO_LONGER_NEEDED(scriptnames[i].name);
