			...
			CefRefPtr<CefRequestContext> request_context) OVERRIDE
		{
			...
			if (command_line->HasSwitch(switches::kLoadExtension))
			{
				...
				if (!extension_path.empty())
				{
					...
					{
						if (!part.empty())
							ExtensionUtil::LoadExtension(request_context, part, this);
					}
				}
			}
		}
...
scoped_refptr<RootWindow> RootWindowManager::CreateRootWindowAsExtension(
	...
	bool with_osr)
{
	const std::string& extension_url = ExtensionUtil::GetExtensionURL(extension);
...
void RootWindowManager::AddExtension(CefRefPtr<CefExtension> extension)
{
	...
	if (ExtensionUtil::GetExtensionURL(extension).empty())
		return;
